(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('videoChatApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-videoChatApp-error');
                var entityKey = httpResponse.headers('X-videoChatApp-params');
                if (errorHeader) {
